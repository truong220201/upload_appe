...
export default class Listbt extends React.Component {
  constructor(props) {
    ...
    this.nvt = navigation;
    const { listt, uid, email, id } = route.params;
    this.uid = uid;
    this.id = id;
    this.email = email;
  ...
  async listenForItems(itemRef) {
    ...
    const querySnapshot = await getDocs(
      collection(db, "Category_mo_ta_chi_tiet")
    );
      ...
      [...Array(7)].map((o, n) => {
        if (this.thing[n] == `${doc.data().Id_category_dvkt}`) {
          this.setState({
            ...
            item: [
              ...this.state.item,
              `${doc.data().Name.slice(3).split("_").join("")}`,
            ],
        ...
        >
          ...
          {this.state.isLoading ? (
            ...
            <View style={styles.vw2}>
              ...
              >
                <TouchableOpacity
                  onPress={() => this.btntieptuc(this.state.savet, this.uid)}
